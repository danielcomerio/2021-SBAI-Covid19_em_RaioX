...
models = {
    "resnet": {
        "normal": "1619763157.5574179_resnetNormal\model_resnetNormal.h5",
        "processed": "1619762983.280438_resnetProcessed\model_resnetProcessed.h5"
    },
    "mobilenet": {
        "normal": "1620259269.1216702_mobilenetNormal\model_mobilenetNormal.h5",
        "processed": "1619632577.0899992_mobilenetProcessed\model_mobilenetProcessed.h5"
    },
    "efficientnet": {
        "normal": "1620057149.6360557_efficientnetNormal\model_efficientnetNormal.h5",
        "processed": "1620057738.500225_efficientnetProcessed\model_efficientnetProcessed.h5"
    },
    "inception": {
        "normal": "1620193229.588741_inceptionNormal\model_inceptionNormal.h5",
        "processed": "1620315625.5158389_inceptionProcessed\model_inceptionProcessed.h5"
    },
}


def get_img_array(img_path, size=(250, 250)):
    # `img` is a PIL image of size 250x250
...
def get_integrated_gradient_image(img_path):
    ...
    vis = GradVisualizer()
    img_points_integrated = vis.visualize(
        image=orig_img,
    ...
    img_area_integrated = vis.visualize(
        image=orig_img,
        gradients=grads[0].numpy(),
        integrated_gradients=igrads.numpy(),
    ...
    return img_points_integrated, img_area_integrated


def parse_command_line_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("dataset", help="path to the dataset", type=str)
    parser.add_argument("models", help="path to the models", type=str)
...
def main():
    ...
    for name in models:
        ...
        for typ in models_type:
            os.mkdir(os.path.join(args.dataset, OUTPUT_DIR, name, typ))
            model = load_model(os.path.join(args.models, models[name][typ]))

            for i in range(len(list_path_old)):
                oldPath = list_path_old[i]
                newPathPointsIntegrated = os.path.join(
                    args.dataset, OUTPUT_DIR, name, typ, image_dirs[i], "points")
                newPathAreaIntegrated = os.path.join(
                    args.dataset, OUTPUT_DIR, name, typ, image_dirs[i], "area")
                os.makedirs(newPathPointsIntegrated)
                os.makedirs(newPathAreaIntegrated)

                for imagePath in os.listdir(oldPath):
                    # imagePath contains name of the image
                    inputPath = os.path.join(oldPath, imagePath)

                    img_points_integrated, img_area_integrated = get_integrated_gradient_image(
                        inputPath)

                    # fullOutPath contains the path of the output
                    fullOutPath = os.path.join(
                        newPathPointsIntegrated, imagePath)
                    img_points = Img.fromarray(img_points_integrated)
                    img_points.save(fullOutPath)

                    fullOutPath = os.path.join(
                        newPathAreaIntegrated, imagePath)
                    img_area = Img.fromarray(img_area_integrated)
                    img_area.save(fullOutPath)
